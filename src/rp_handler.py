...
def process_output_images(outputs, job_id):
  ...
  for node_id, node_output in outputs.items():
    if "images" in node_output:
      print('node_output["images"]', node_output["images"])
  ...
  if output_images:
    ...
    for output_image in output_images:
      ...
      if not os.path.exists(local_image_path):
        print(f"runpod-worker-comfy - file not found: {local_image_path}")
        continue
      ...
      try:
        if os.environ.get("BUCKET_ENDPOINT_URL", False):
          ...
          if image:
            ...
            print("runpod-worker-comfy - failed to upload image to AWS S3")
            continue
        else:
          # base64 image
          ...
          if image:
            ...
            print("runpod-worker-comfy - failed to encode image to base64")
            continue

        image_urls.append(image)
      except Exception as e:
        print(f"runpod-worker-comfy - error processing image {local_image_path}: {str(e)}")
    return {
        "status": "success",
        "message": image_urls,
    }
